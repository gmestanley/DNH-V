...
void scanner::advance()
{
	...
	case L'.':
		...
		if (ch == L'.') {
			...
			std::wstring error;
			error += L"Single periods must not be used in this script.\r\n";
	...
	case L'\\': {
		...
		default: {
			std::wstring error;
			error += L"A character in particular is acting weirdly. (Are you not forgetting the \"...\"?)\r\n";
...
parser::symbol* parser::search(std::string const& name)
{
	for (int i = frame.size() - 1; i >= 0; --i) {
		Logger::WriteTop(L"Internal processing|" + to_wide(std::to_string(frame[i].find(name) != frame[i].end())));
		Logger::WriteTop(L"Internal processing|" + to_wide(name));
...
void parser::parse_clause(script_engine::block* block)
{
	if (lex->next == tk_real) {
		...
		for (int i = frame.size() - 1; i >= 0; --i) {
			Logger::WriteTop(L"External processing|" + to_wide(std::to_string(s.level)));
			Logger::WriteTop(L"External processing|" + to_wide(std::to_string(s.variable)));
