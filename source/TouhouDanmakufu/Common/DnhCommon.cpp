...
DnhConfiguration::DnhConfiguration()
{
	...
	mapKey_[ESimpleInput::KEY_LEFT] = new VirtualKey(DIK_LEFT, 0, 0); //キーボードとジョイパッド「←」を登録/Register Keyboard and Joypad "←"
	mapKey_[ESimpleInput::KEY_RIGHT] = new VirtualKey(DIK_RIGHT, 0, 1); //キーボードとジョイパッド「→」を登録/Register Keyboard and Joypad "→"
	mapKey_[ESimpleInput::KEY_UP] = new VirtualKey(DIK_UP, 0, 2); //キーボードとジョイパッド「↑」を登録/Register Keyboard and Joypad "↑"
	mapKey_[ESimpleInput::KEY_DOWN] = new VirtualKey(DIK_DOWN, 0, 3); //キーボードとジョイパッド「↓」を登録/Register Keyboard and Joypad "↓"

	mapKey_[ESimpleInput::KEY_OK] = new VirtualKey(DIK_Z, 0, 5);
	mapKey_[ESimpleInput::KEY_CANCEL] = new VirtualKey(DIK_X, 0, 6);

	mapKey_[ESimpleInput::KEY_SHOT] = new VirtualKey(DIK_Z, 0, 5);
	mapKey_[ESimpleInput::KEY_BOMB] = new VirtualKey(DIK_X, 0, 6);
	mapKey_[ESimpleInput::KEY_SLOWMOVE] = new VirtualKey(DIK_LSHIFT, 0, 7);
	mapKey_[ESimpleInput::KEY_USER1] = new VirtualKey(DIK_C, 0, 8);
	mapKey_[ESimpleInput::KEY_USER2] = new VirtualKey(DIK_V, 0, 9);

	mapKey_[ESimpleInput::KEY_PAUSE] = new VirtualKey(DIK_ESCAPE, 0, 10);
